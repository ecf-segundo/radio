...
# Busca no site www.radio.net pelas estações de São Paulo
RADIOS=`curl -s https://br.radio.net/country/brazil/sao-paulo | grep -o -e href=\"https://br.radio.net/s/[a-z,0-9]*\" | cut -d '"' -f2`
#RADIOS=`curl -s https://br.radios.net/ | grep "https://www.radios.com.br/aovivo/" | grep btn | sed 's/ /_/g' | sed 's/"_/" /g' | sed 's/data-toggle="tooltip" //g' |cut -d " " -f2- | sed 's/href="https/https/g' | sed 's/"//g' | sed 's/>/ /g' | sed 's/</ /g' | awk '{print $1,$3}' | sed 's/ /,/g'`
